...
const templates = {
  classic: { cardBg: 'bg-white', questionBg: 'bg-slate-50' },
  modern: { cardBg: 'bg-gradient-to-br from-purple-50 to-pink-50', questionBg: 'bg-white/80 backdrop-blur' },
  minimal: { cardBg: 'bg-slate-900', questionBg: 'bg-slate-800' }
}
...
export default function FormEditPage() {
  ...
  const [formType, setFormType] = useState<'SINGLE_PAGE' | 'INTERACTIVE'>('SINGLE_PAGE')
  ...
  const handleImageUpload = async (file: File, type: 'header' | 'background') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (type === 'header') setHeaderImage(path)
                ...
                <DialogContent className="max-w-4xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {(['classic', 'modern', 'minimal'] as Template[]).map((t) => (
                      <button
                        key={t}
                        onClick={() => { setTemplate(t); setShowTemplateSelector(false) }}
                        className={`p-4 border-2 rounded-lg hover:shadow-lg transition-all ${template === t ? 'border-blue-600 bg-blue-50' : 'border-slate-200'}`}
                      >
                        <div className={`aspect-video rounded mb-3 ${templates[t].cardBg}`} />
                        <h3 className="font-semibold capitalize">{t}</h3>
                      </button>
                    ))}
                  </div>
                </DialogContent>
                  ...
                  <Card key={q.id} className={`shadow-lg ${templates[template].cardBg} ${template === 'minimal' ? 'text-white' : ''}`}>
                    <CardContent className="p-4 md:p-6 space-y-4">
                      <div className="space-y-2">
                        ...
                        {q.description && <p className={`text-sm ${template === 'minimal' ? 'text-slate-400' : 'text-slate-600'}`}>{q.description}</p>}
                      </div>
                ...
                {questions.map((question, index) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 space-y-3">
                        {editingQuestion === question.id ? (
                          <>
                            ...
                            <Textarea value={question.description || ''} onChange={(e) => updateQuestion(question.id, 'description', e.target.value)} />
                      ...
                      <div className="flex gap-2">
                        {editingQuestion !== question.id && (
                          <Button variant="ghost" size="sm" onClick={() => setEditingQuestion(question.id)} className="text-blue-600">
                            <Edit2 className="w-4 h-4" />
                          </Button>
                        )}
                        <Button variant="ghost" size="sm" onClick={() => removeQuestion(question.id)} className="text-red-600">
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      </div>
                    </div>
