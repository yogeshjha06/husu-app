...
export default function ResourcesPage() {
  ...
  // const [filteredResources, setFilteredResources] = useState<Resource[]>([]) // Removed client-side filter state for now or keep it if we want to filter the current page
  ...
  // Client-side filtering on the *current page* of data (optional, or we can move this to backend later)
  ...
  const fetchResources = async (page: number) => {
    setLoading(true)
    try {
      const res = await fetch(`/api/org-admin/resources?page=${page}&limit=${pagination.limit}`)
      if (res.ok) {
  ...
  // ... (keep getTypeColor and other render logic) ...
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <div className="px-3 py-1 rounded-full bg-indigo-600/10 border border-indigo-600/20 flex items-center gap-2">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest text-indigo-600">Learning Center</span>
            </div>
          </div>
          ...
          <p className="text-slate-500 text-lg font-medium max-w-2xl">
            Curated materials for training, wellness, and compliance.
          </p>
        </motion.div>
